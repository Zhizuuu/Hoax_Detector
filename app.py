...
import toml
import logging
import os, re, time, json, requests, nltk, numpy as np, uuid
...
# ╭─[ F. MODEL ]───────────────────────────────────────────────────╮
# =========[ F. MODEL – versi XGBoost ]=========
def predict_one(text: str):
    """
    Prediksi HOAX/FAKTA menggunakan TF-IDF + XGBoost.
    """
    # 1. Preprocess teks
    pre  = preprocess_text(clean_text(text))

    # 2. Ubah ke vektor
    Xvec = vectorizer.transform([pre])  # Sparse matrix

    # 3. Prediksi probabilitas HOAX (kelas 1)
    if hasattr(xgb_clf, "predict_proba"):
        probs = xgb_clf.predict_proba(Xvec)[0]  # [p_fakta, p_hoax]
        prob_hoax = probs[1]
    else:
        prob_hoax = float(xgb_clf.predict(Xvec)[0])  # fallback

    label = "HOAX" if prob_hoax >= 0.5 else "FAKTA"
    return label, prob_hoax
